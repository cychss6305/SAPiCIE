...
def evaluate(args, logger, dataloader, classifier, model):
    ...
    # if not os.path.exists(args.save_root+"K_train/K_test/pred/"):
    #     os.makedirs(args.save_root+"K_train/K_test/pred/")
    model.eval()
    classifier.eval()
    with torch.no_grad():
        for i, (indice, image, label) in enumerate(dataloader):
            ...
            preds=probs.topk(1, dim=1)[1]
            
            # for jndex,j in enumerate(indice):
            #      im=torch.argmax(probs[jndex],dim=0).cpu().numpy().astype(np.uint8)
            #      imageio.imwrite("./pred/IIC/"+dataloader.dataset.imdb[j]+".png",im)
            preds =preds.view(B, -1).cpu().numpy()
