...
from data.voc_train_dataset import TrainVOC
from data.voc_eval_dataset import EvalVOC
...
def get_dataset(args, mode, inv_list=[], eqv_list=[]):
    if args.cityscapes:
        ...
        if mode == 'train':
            dataset = TrainVOC(args.data_root, labeldir=args.save_model_path, res1=args.res1, res2=args.res2, 
                                      split='train', mode='compute', inv_list=inv_list, eqv_list=eqv_list, scale=(args.min_scale, 1))
        elif mode == 'train_val':
            dataset = EvalVOC(args.data_root, res=args.res, split='val', mode='test')
        elif mode == 'eval_val':
            dataset = EvalVOC(args.data_root, res=args.res, split=args.val_type,mode='test', label=False)
        elif mode == 'eval_test':
            dataset = EvalVOC(args.data_root, res=args.res, split='val', mode='test')
    else:
        if mode == 'train':
