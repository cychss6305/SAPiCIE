...
class RandomGaussianBlur(object):
    ...
    def __call__(self, index, image):
        if self.plist[index] < self.p_ref:
            ...
            # lap = Image.fromarray(cv2.cvtColor(lap,cv2.COLOR_BGR2RGB))
            # return lap
            
            # canny = cv2.Canny(blurred, 80, 150)
            # canny = Image.fromarray(cv2.cvtColor(canny,cv2.COLOR_BGR2RGB))
            # return canny
            ...
            # sobel= Image.fromarray(cv2.cvtColor(dst,cv2.COLOR_BGR2RGB))
            # return sobel
        else:
            return image
