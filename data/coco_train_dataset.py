...
class TrainCOCO(data.Dataset):
    ...
    def transform_image(self, index, image):
        ...
        if self.mode == 'compute':
            if self.view == 1:
                ...
                image = self.transform_inv(index, image, 2)
                image = self.transform_tensor(image)
            else:
            ...
            image3 = self.transform_inv(index, image, 2)
            image3 = self.transform_tensor(image3)
    ...
    def init_transforms(self):
        ...
        self.random_color_brightness = [RandomColorBrightness(x=0.3, p=0.8, N=N) for _ in range(3)] # Control this later (NOTE)]
        self.random_color_contrast   = [RandomColorContrast(x=0.3, p=0.8, N=N) for _ in range(3)] # Control this later (NOTE)
        self.random_color_saturation = [RandomColorSaturation(x=0.3, p=0.8, N=N) for _ in range(3)] # Control this later (NOTE)
        self.random_color_hue        = [RandomColorHue(x=0.1, p=0.8, N=N) for _ in range(3)]      # Control this later (NOTE)
        self.random_gray_scale    = [RandomGrayScale(p=0.2, N=N) for _ in range(3)]
        self.random_gaussian_blur = [RandomGaussianBlur(sigma=[.1, 2.], p=0.5, N=N) for _ in range(3)]
