...
def parse_arguments():
    ...
    parser.add_argument('--batch_size_train', type=int, default=8) #16
    parser.add_argument('--batch_size_test', type=int, default=32) #32
...
def train(args, logger, dataloader, model, classifier1, classifier2, criterion1, criterion2, optimizer, epoch):
    ...
    for i, (indice, input1, input2, label1, label2) in enumerate(dataloader):
        ...
        label1 ,label2= label1.cuda(non_blocking=True),label2.cuda(non_blocking=True)
        input1 = eqv_transform_if_needed(args, dataloader, indice, input1)
...
if __name__=='__main__':
    ...
    args.save_model_path = os.path.join(args.save_root,'K_train')
    args.save_eval_path  = os.path.join(args.save_model_path, 'K_test')
